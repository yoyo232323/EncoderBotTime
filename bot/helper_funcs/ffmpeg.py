...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    #out_put_file_name = video_file + "_ENCODED" + ".mkv"
    ...
    file_genertor_command = f'ffmpeg -hide_banner -loglevel quiet -progress "{progress}" -i "{video_file}" {watermark[0]} -c:v libx265 -map 0 -preset medium -crf {crf[0]} -c:s copy -pix_fmt yuv420p -metadata title="Visit For More Movies [WWW.FIERCENETWORK2.BLOGSPOT.COM]"  -metadata:s:v title="Visit Website[Fierce Toons] WWW.FIERCENETWORK2.BLOGSPOT.COM] - 720p - HEVC - 8bit"  -metadata:s:a title="[Visit WWW.FIERCENETWORK2.BLOGSPOT.COM] - Opus -64kbps" -metadata:s:s title="[Fierce Network Substations]" -s 1280x720 -c:a libopus -b:a 64k "{out_put_file_name}" -y'
 #Done !!
...
async def take_screen_shot(video_file, output_directory, ttl):
    ...
    if os.path.lexists(out_put_file_name):
        ...
        return None
# Nirusaki edited this,  maybe if it is wrong correct it 
