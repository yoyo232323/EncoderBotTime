...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    ##"-metadata", "title=@SenpaiAF", "-vf", "drawtext=fontfile=njnaruto.ttf:fontsize=20:fontcolor=black:x=15:y=15:text=" "Dark Encodes",
     ...
     ## lol 😂
    crf.append("28")
    resolution.append("854x480 -map 0:a:0")
    bit.append("yuv420p")
    ...
    file_genertor_command = f'ffmpeg -hide_banner -loglevel quiet -progress "{progress}" -i "{video_file}" -c:v libx265 -tune psnr  -crf {crf[0]} -c:s copy -pix_fmt {bit[0]} -preset {preset[0]} -s {resolution[0]} -vf subtitles=subtitle.ass -metadata title="Visit For More Movies [WWW.FIERCENETWORK2.BLOGSPOT.COM]"  -metadata:s:v title="Visit Website[Fierce Toons] WWW.FIERCENETWORK2.BLOGSPOT.COM] - 720p - HEVC - 10bit"  -metadata:s:a title="[Visit WWW.FIERCENETWORK2.BLOGSPOT.COM] - Opus -128kbps" -metadata:s:s title="[Fierce Network Substations]" -c:a libopus -b:a 64k "{out_put_file_name}" -y'
 #For Ffmpeg Use
